...
OCLData initOCL(const char* kernelFileName, const char* kernelName)
{
	...
	try
	{
		...
		for (cl::Platform& p : platforms)
		{
			...
			if (!devs.empty())
			{
				...
				for (cl::Device& dev : devs)
				{
					...
					cl_uint maxDims;
					dev.getInfo(CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, &maxDims);
					vector<size_t> workitemSize;
					dev.getInfo(CL_DEVICE_MAX_WORK_ITEM_SIZES, &workitemSize);
					size_t workgroupSize;
					dev.getInfo(CL_DEVICE_MAX_WORK_GROUP_SIZE, &workgroupSize); // product of all dimension
		...
		// read in file and store it in string prog
		file.close();
		...
		ocl.m_kernel = cl::Kernel(program, kernelName);
		// create the s_kernel: must be the name of the s_kernel in the cl file
...
void matMultGPU(OCLData& ocl, const int* a, const int* b, int* const c, const int n)
{
	...
	size_t mem_size = sizeof(int) * n * n;
	int errcode;


	cl::Buffer d_A(ocl.m_context, CL_MEM_READ_ONLY, mem_size, NULL, &errcode);
	cl::Buffer d_B(ocl.m_context, CL_MEM_READ_ONLY, mem_size, NULL, &errcode);
	cl::Buffer d_C(ocl.m_context,CL_MEM_READ_WRITE, mem_size, NULL, &errcode);
	//cl::Buffer d_B(ocl.m_context, CL_MEM_READ_ONLY, mem_size, brow, &errcode);

	int a_cpy = *a;


	ocl.m_queue.enqueueReadBuffer(d_A, true, 0, mem_size, &a);
	ocl.m_queue.enqueueReadBuffer(d_A, true, 0, mem_size, &b);
	ocl.m_queue.enqueueWriteBuffer(d_A, true, 0, mem_size, &c);

	ocl.m_kernel.setArg(3, n);

	ocl.m_queue.enqueueNDRangeKernel(ocl.m_kernel, cl::NullRange, cl::NDRange(n, n));
}
